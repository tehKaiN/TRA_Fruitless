...
#define S_FUNCTION_NAME simplified
...
//    }
    ...
    const char* hmfFilePath = "simplified.hmf";
    double startT, stopT;
    ...
    if (pComponentSystem==0)
    {
        ...
        ssSetErrorStatus(S,"Error could not open model: simplified.hmf");
        return;
    }
    startT = ssGetTStart(S);
    stopT = ssGetTFinal(S);
    pComponentSystem->setDesiredTimestep(0.001);
    mexCallMATLAB(0, 0, 0, 0, "simplifiedMaskSetup");                              //Run the port label script
}
...
static void mdlInitializeSampleTimes(SimStruct *S)
{
    ...
    pInputNode0 = pComponentSystem->getSubComponent("Torque")->getSafeNodeDataPtr("out", 0);
    pOutputNode0 = pComponentSystem->getSubComponent("omega3")->getSafeNodeDataPtr("in", 0);
