...
#define S_FUNCTION_NAME simpleton
...
//    }
    ...
    const char* hmfFilePath = "simpleton.hmf";
    double startT, stopT;
    ...
    if (pComponentSystem==0)
    {
        ...
        ssSetErrorStatus(S,"Error could not open model: simpleton.hmf");
        return;
    }
    startT = ssGetTStart(S);
    stopT = ssGetTFinal(S);
    pComponentSystem->setDesiredTimestep(0.001);
    mexCallMATLAB(0, 0, 0, 0, "simpletonMaskSetup");                              //Run the port label script
}
