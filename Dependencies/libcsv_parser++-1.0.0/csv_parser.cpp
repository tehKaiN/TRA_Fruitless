	...
	if (*line_length > 0)
	{
		...
		while(char_pos < *line_length)
		{
			...
			if (curr_char == field_term_char)
			{
                ...
                if (field_width == 0) //The if statement is a patch found at project homepage: http://code.google.com/p/csv-parser-cplusplus/issues/attachmentText?id=1&aid=-7075719710590776640&name=patch&token=2BY_GIGvHipZcwUScQ5eGKFYysE%3A1331646940967
                {
                    CSV_PARSER_FREE_BUFFER_PTR(field);
                    return;
                }
			...
			} else if (curr_char == line_term_char)
			{
                ...
                if (field_width == 0) //The if statment is a patch found at project homepage: http://code.google.com/p/csv-parser-cplusplus/issues/attachmentText?id=1&aid=-7075719710590776640&name=patch&token=2BY_GIGvHipZcwUScQ5eGKFYysE%3A1331646940967
                {
                    CSV_PARSER_FREE_BUFFER_PTR(field);
                    return;
                }
